...
class CashierRegistration:
    def __init__(self, master, *args, **kwargs):
        ...
        # button
        self.button_add = Button(master, text="Add", command=self.addCashier, font=("arial 13 bold"), width=8, height=1,
                                 bg="steelblue", fg="white", )
        self.button_add.place(x=50, y=510)

        self.button_search = Button(master, text="Search", command=self.searchCashier, font=("arial 13 bold"), width=8,
                                    height=1, bg="steelblue", fg="white", )
        self.button_search.place(x=155, y=510)

        self.button_update = Button(master, text="Update", command=self.updateCashier, font=("arial 13 bold"), width=8,
                                    height=1, bg="steelblue", fg="white", )
        self.button_update.place(x=260, y=510)

        self.button_delete = Button(master, text="Delete", command=self.deleteCashier, font=("arial 13 bold"), width=8,
                                    height=1, bg="steelblue", fg="white", )
        ...
        self.btn_show_detail = Button(self.table_frame, command=self.cashierDetail, text=" Show", width="7", height=1,
                                      font=("arial 12 bold"), bg="green", fg="black", )
        self.btn_show_detail.place(x=300, y=10)

        self.btn_show_detailAll = Button(self.table_frame, command=self.cashierDetailAll, text=" Show All", width="7",
                                         height=1, font=("arial 12 bold"), bg="green", fg="black", )
        ...
        self.trv['columns'] = (
        'Id', 'Name', 'Username', 'Password', 'BirthDate', 'Address', 'Phone', 'Email', 'Remarks', 'AddedOn',
        'ModifiedOn')
    ...
    def addCashier(self, *args, **kwargs):
        ...
        if self.cashier_name != '' and self.cashier_username != '' and self.cashier_password != '' and self.cashier_dob != "" and self.cashier_address != "" and self.cashier_phone != '' and self.cashier_email != "":
            ...
            c.execute(sql, (
            self.cashier_name, self.cashier_username, self.cashier_password, self.cashier_dob, self.cashier_address,
            self.cashier_phone, self.cashier_email, self.cashier_remarks, date))
            conn.commit()
    ...
    def searchCashier(self, *args, **kwargs):
        ...
        if self.entry_cashier_id.get() != '' and [self.result for self.result in self.result if
                                                  self.result[0] == int(self.entry_cashier_id.get())]:
            sql = "SELECT * FROM cashier WHERE cashier_id=?"
    ...
    def updateCashier(self, *args, **kwargs):
        ...
        if self.cashier_name != '' and self.cashier_username != '' and self.cashier_password != '' and self.cashier_dob != "" and self.cashier_address != "" and self.cashier_phone != '' and self.cashier_email != "":
            ...
            c.execute(sql, (
            self.u1, self.u2, self.u3, self.u4, self.u5, self.u6, self.u7, self.u8, date, self.entry_cashier_id.get()))
