...
class ManageProduct:
    def __init__(self, master, *args, **kwargs):
        ...
        # button
        self.button_add = Button(master, text="Add", font=("arial 13 bold"), width=8, height=1, bg="steelblue",
                                 fg="white", command=self.addProduct)
        self.button_add.place(x=50, y=640)

        self.button_add = Button(master, text="Search", font=("arial 13 bold"), width=8, height=1, bg="steelblue",
                                 fg="white", command=self.searchProduct)
        self.button_add.place(x=155, y=640)

        self.button_add = Button(master, text="Update", font=("arial 13 bold"), width=8, height=1, bg="steelblue",
                                 fg="white", command=self.updateProduct)
        self.button_add.place(x=260, y=640)

        self.button_add = Button(master, text="Delete", font=("arial 13 bold"), width=8, height=1, bg="steelblue",
                                 fg="white", command=self.deleteProduct)
        ...
        self.btn_show_detail = Button(self.table_frame, text=" Show", width="7", height=1, font=("arial 12 bold"),
                                      bg="green", fg="black", command=self.show_detail)
        self.btn_show_detail.place(x=300, y=10)

        self.btn_show_detailAll = Button(self.table_frame, text=" Show All", width="7", height=1,
                                         font=("arial 12 bold"), bg="green", fg="black", command=self.show_detailAll)
        self.btn_show_detailAll.place(x=430, y=10)

        self.labal_date_today = Label(self.table_frame, text="Today's Date:" + str(date), font=(" arial 14 bold"),
                                      bg="steelblue", fg="black")
        ...
        self.trv = ttk.Treeview(self.trv_frame, height="20", xscrollcommand=self.horizontalScrollbar,
                                yscrollcommand=self.verticalScrollbar)
        self.trv.place(x=20, y=50)
        self.trv['columns'] = (
        'Id', 'Product', 'Stocks', 'CostPrice', 'Vender', 'Phone', 'Mfd.Date', 'Exp.Date', 'Max.Ret.Price',
        'MarkedPrice', 'Remarks', 'AddedOn', 'ModifiedOn')
    ...
    def addProduct(self, *args, **kwargs):
        ...
        if self.product_name != '' and self.number_of_stocks != '' and self.cost_price != '' and self.vender_name != '' and self.vender_phone != '' and self.manufacture_date != '' and self.expiry_date != '' and self.maximum_retail_price != '' and self.marked_price != '':
            ...
            c.execute(sql, (
            self.product_name, self.number_of_stocks, self.cost_price, self.vender_name, self.vender_phone,
            self.manufacture_date, self.expiry_date, self.maximum_retail_price, self.marked_price, self.remarks, date))
            conn.commit()
    ...
    def searchProduct(self, *args, **kwargs):
        ...
        if self.entry_product_id.get() != '' and [self.result for self.result in self.result if
                                                  self.result[0] == int(self.entry_product_id.get())]:
    ...
    def updateProduct(self, *args, **kwargs):
        ...
        if self.product_name != '' and self.number_of_stocks != '' and self.cost_price != '' and self.vender_name != '' and self.vender_phone != '' and self.manufacture_date != '' and self.expiry_date != '' and self.maximum_retail_price != '' and self.marked_price != '':
            ...
            c.execute(sql, (
            self.u1, self.u2, self.u3, self.u4, self.u5, self.u6, self.u7, self.u8, self.u9, self.u10, date,
    ...
    def show_detail(self, *argts, **kwargs):
        ...
        if len(self.rows) != 0:
            ...
            for row in self.rows:
                if row[2] < 5:
                    self.trv.insert('', self.i, text='', values=(
