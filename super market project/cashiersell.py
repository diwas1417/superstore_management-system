...
class Application:
    def __init__(self, master, *args, **kwargs):
        ...
        # for left frame
        self.heading = Label(self.left_frame, text="Search for the product", font=("arial 20 bold"), bg="lightblue",
                             fg="steelblue")
        self.heading.place(x=5, y=20)

        self.labal_product_id = Label(self.left_frame, text=("Enter Product Id"), font=("arial 15 bold"),
        ...
        self.btn_search = Button(self.left_frame, text=" Search", width="7", height=1, font=("arial 12 bold"),
                                 bg="green", fg="black", command=self.see_details)
        ...
        self.labal_number_of_stocks = Label(self.left_frame, text=(""), font=("arial 15 bold"), bg="lightblue",
                                            fg="black")
        ...
        self.btn_show_detail = Button(self.middle_frame, text=" Show", width="7", height=1, font=("arial 12 bold"),
                                      bg="green", fg="black", command=self.show_detail)
        self.btn_show_detail.place(x=300, y=10)

        self.btn_show_detailAll = Button(self.middle_frame, text=" Show All", width="7", height=1,
                                         font=("arial 12 bold"), bg="green", fg="black", command=self.show_detailAll)
        ...
        self.trv = ttk.Treeview(self.table_frame, height="25", xscrollcommand=self.horizontalScrollbar,
                                yscrollcommand=self.verticalScrollbar)

        self.trv['columns'] = (
        'Id', 'Product', 'Stocks', 'CostPrice', 'Mfd.Date', 'Exp.Date', 'Max.Ret.Price', 'MarkedPrice')
        ...
        # for the right frame
        self.labal_date_today = Label(self.right_frame, text="Today's Date:" + str(date), font=(" arial 10 bold"),
                                      bg="lightblue", fg="black")
        ...
        self.tab_quantity = Label(self.right_frame, text="Quantity", font=(" arial 11 bold"), bg="lightblue",
                                  fg="black")
    ...
    def see_details(self, *args, **kwargs):
        ...
        self.labal_quantity = Label(self.left_frame, text=("Quantity of Product"), font=("arial 13 bold"),
                                    bg="lightblue", fg="black")
        ...
        self.labal_discount = Label(self.left_frame, text=("Discount Amount"), font=("arial 13 bold"), bg="lightblue",
                                    fg="black")
        ...
        # to add to bill
        self.btn_add_to_cart = Button(self.left_frame, text=" Add to Cart", width="11", height=1,
                                      font=("arial 11 bold"), bg="green", fg="black", command=self.add_to_cart)
        self.btn_add_to_cart.place(x=225, y=380)

        # for the amount change
        self.labal_given_amount = Label(self.left_frame, text=("Given Amount"), font=("arial 13 bold"), bg="lightblue",
                                        fg="black")
        ...
        self.btn_make_change = Button(self.left_frame, text=" Change", width="7", height=1, font=("arial 11 bold"),
                                      bg="yellow", fg="black", command=self.change)
        self.btn_make_change.place(x=255, y=420)

        # to generate the bill
        self.btn_generate_bill = Button(self.left_frame, text=" Generate Bill", width="25", height=2,
                                        font=("arial 15 bold"), bg="red", fg="black", command=self.generate_bill)
    ...
    def add_to_cart(self, *args, **kwargs):
        ...
        if self.quantity > int(self.get_number_of_stocks):
            tkinter.messagebox.showinfo("Error!", " We have only limited Products.\n Number of" + str(
                self.get_product_name) + " is " + str(self.get_number_of_stocks))
        else:
            ...
            for self.p in pname:
                self.tempname = Label(self.right_frame, text=str(pname[self.coun]), font=("arial 10 bold"),
                                      bg="lightgreen", fg="black")
                self.tempname.place(x=20, y=self.yin)
                product_list.append(self.tempname)

                self.temprate = Label(self.right_frame, text=str(prate[self.coun]), font=("arial 10 bold"),
                                      bg="lightgreen", fg="black")
                self.temprate.place(x=160, y=self.yin)
                product_list.append(self.temprate)

                self.tempquantity = Label(self.right_frame, text=str(pquantity[self.coun]), font=("arial 10 bold"),
                                          bg="lightgreen", fg="black")
                self.tempquantity.place(x=230, y=self.yin)
                product_list.append(self.tempquantity)

                self.tempamount = Label(self.right_frame, text=str(pamount[self.coun]), font=("arial 10 bold"),
                                        bg="lightgreen", fg="black")
    ...
    def change(self, *args, **kwargs):
        ...
        if self.bill_total_amount <= self.bill_given_amount:
            ...
            tkinter.messagebox.showinfo("Error!", "The Given Amount is insufficient.\n Rs." + str(
                self.bill_change_amount) + " is Required!")
        else:
            self.entry_given_amount.focus()
            self.entry_given_amount.delete(0, END)
            tkinter.messagebox.showinfo("Error!",
                                        "The Given Amount is insufficient.\n Or \n More Amount is Required!  ")
    ...
    def generate_bill(self, *args, **kwargs):
        ...
        r = 0
        for t in pname:
            f.write("\n\t" + str(r + 1) + "\t" + str(pname[r][:10]) + "\t\t" + str(prate[r]) + "\t\t" + str(
                pquantity[r]) + "\t\t" + str(pamount[r]))
            r += 1
        f.write(
            "\n\t---------------------------------------------------------------------------\n\t\t\t\t\t\tYour Total bill is Rs." + str(
                sum(pamount)))
        ...
        for i in pname:
            ...
            c.execute(sql2, (
            pid[self.x], pname[self.x], pdiscount[self.x], prate[self.x], pquantity[self.x], pamount[self.x], date))
            conn.commit()
